...
/* This code is used in the growing process in the Boomerang. A large part of this code was taken from C code available in the R CRAN package 'survival' on November 19, 2014. I have modified the code for use in the Boomerang algorithm. For the freely available Boomerang library, please visit www.github.com/sedevlin.
 */
 

double cpeftnE(double *time, int *status, double *xvec, double *currentlevels, int nused, int nvar,
...
SEXP combinedEXPANDMAX(SEXP existClass, SEXP uniqueExist, SEXP markermatrix, SEXP ux, SEXP jx,
                       SEXP minatrisk2, SEXP minevent2, SEXP time2,   SEXP status2,
                       SEXP offset2, SEXP weights2,
                       SEXP strata2,   SEXP eps2, SEXP toler2){
    
    int i,j,k,minatrisk,minevent;
    int protected=0;
    int currentnum;
    ...
    minatrisk= asInteger(minatrisk2);
    minevent = asInteger(minevent2);
    ...
    double *uxPtr,uxtemp;
    int *jxPtr, jxtemp, jxlength;
    uxPtr=REAL(ux);
    jxPtr=INTEGER(jx);
    jxlength= length(jx);
    
    currentnum=length(uniqueExist);
    
    double *markermatrixPtr;
    PROTECT(markermatrix = duplicate(markermatrix));
    protected++;
    markermatrixPtr=REAL(markermatrix);
    
    int *validexpansionPtr;
    SEXP validexpansion;
    PROTECT(validexpansion = allocVector(INTSXP, currentnum*jxlength ));
    protected++;
    validexpansionPtr=INTEGER(validexpansion);
    
    SEXP expandedmatrix;
    double *expandedmatrixPtr;
    PROTECT(expandedmatrix = allocVector(REALSXP, nused*currentnum*jxlength));
    protected++;
    expandedmatrixPtr=REAL(expandedmatrix);
    
    SEXP Grplist, jxlist, uxlist;
    int *GrplistPtr, *jxlistPtr;
    double *uxlistPtr;
    PROTECT(Grplist = allocVector(INTSXP, currentnum*jxlength));
    protected++;
    PROTECT(jxlist = allocVector(INTSXP, currentnum*jxlength));
    protected++;
    PROTECT(uxlist = allocVector(REALSXP, currentnum*jxlength));
    protected++;
    GrplistPtr=INTEGER(Grplist);
    jxlistPtr=INTEGER(jxlist);
    uxlistPtr=REAL(uxlist);
    
    
    int n21, n22, ll;
    int d21, d22,totalvalid;
    
    ll=0;
    totalvalid=0;
    for(i=0; i<currentnum; i++){
        for(j=0; j < jxlength; j++){
            uxtemp = uxPtr[j];
            jxtemp = jxPtr[j];
            n21=0;
            n22=0;
            ...
            d22=0;
            for(k=0; k <nused; k++){
                if (existClassPtr[k] < (i+1)) {
                    expandedmatrixPtr[k+(ll*nused) ]=existClassPtr[k];
                }
                if (existClassPtr[k] > (i+1)) {
                    expandedmatrixPtr[k+(ll*nused)]=existClassPtr[k]+1;
                }
                if (existClassPtr[k]==(i+1)  &&  markermatrixPtr[k+((jxtemp-1)*nused)]  <= uxtemp) {
                    expandedmatrixPtr[k+(ll*nused)]=existClassPtr[k];
                    n21++;
                    if(status[k]==1) d21++;
                }
            ...
            if(n21 >= minatrisk && d21 >= minevent && n22 >= minatrisk && d22 >= minevent){
                validexpansionPtr[ll]=1;
                totalvalid++;
            }else{
                validexpansionPtr[ll]=0;
            }
            ll++;
        }
    }
    ...
    SEXP GrplistEXP, jxlistEXP, uxlistEXP;
    int *GrplistEXPPtr, *jxlistEXPPtr;
    double *uxlistEXPPtr;
    PROTECT(GrplistEXP = allocVector(INTSXP, totalvalid));
    protected++;
    PROTECT(jxlistEXP = allocVector(INTSXP, totalvalid));
    protected++;
    PROTECT(uxlistEXP = allocVector(REALSXP, totalvalid));
    protected++;
    GrplistEXPPtr=INTEGER(GrplistEXP);
    jxlistEXPPtr=INTEGER(jxlistEXP);
    uxlistEXPPtr=REAL(uxlistEXP);
    
    
    SEXP finaldesign;
    double *finaldesignPtr;
    PROTECT(finaldesign = allocVector(REALSXP, nused*totalvalid));
    protected++;
    finaldesignPtr=REAL(finaldesign);
    
    int cnt=0;
    for(i=0; i<ll;i++){
        if(validexpansionPtr[i]==1){
            for(k=0; k <nused; k++){
            finaldesignPtr[k+(cnt*nused)]=expandedmatrixPtr[k+(i*nused)];
            }
            GrplistEXPPtr[cnt]=GrplistPtr[i];
            jxlistEXPPtr[cnt]=jxlistPtr[i];
            uxlistEXPPtr[cnt]=uxlistPtr[i];
            cnt++;
        }
    }
    ...
    for (i=0; i< totalvalid; i++) {
        ...
        templist = cpeftnE(time, status,xvecCOL, currentlevels,  nused,  currentnum,offset, weights,  strata,  eps, toler);
    ...
    double maxux=0;
      for (i=0; i< totalvalid; i++) {
          if( currentCPE[i] >maxCPE){
              maxCPE=currentCPE[i];
              maxcolumn= i;
              maxgrp= GrplistEXPPtr[i];
              maxjx=jxlistEXPPtr[i];
              maxux=uxlistEXPPtr[i];
          }
      }
    ...
    SEXP outdel2;
    PROTECT(outdel2 = allocVector(REALSXP, 4));
    protected++;
    double *outdel;
    outdel =REAL(outdel2);
    outdel[0]=maxCPE;
