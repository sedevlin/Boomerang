...
/* This code is used in the Boomerang algorithm. A large part of this code was taken from C code available in the R CRAN package 'survival' on November 19, 2014. I have modified the code for use in the Boomerang algorithm. For the freely available Boomerang library, please visit www.github.com/sedevlin.
 */

double cpeftnS(double *time, int *status, double *xvec, double *currentlevels, int nused, int nvar,
...
SEXP findmaxCPE(SEXP time2,   SEXP status2,
                SEXP xmatrix2, SEXP currentlevels2, SEXP offset2, SEXP weights2, SEXP strata2,   SEXP eps2,
                SEXP toler2){
    ...
    nvar  = LENGTH(currentlevels2)-1;
    nxmat = ncols(xmatrix2);
    ...
    PROTECT(currentlevels2 = duplicate(currentlevels2));
    protected++;
    currentlevels=REAL(currentlevels2);
   
    
    double **xmatrix;
    PROTECT(xmatrix2 = duplicate(xmatrix2));
    protected++;
    xmatrix = dmatrix(REAL(xmatrix2), nused, nxmat);
    ...
    SEXP currentCPE2;
    PROTECT(currentCPE2 = allocVector(REALSXP, nxmat));
    protected++;
    double *currentCPE;
    ...
    for (i=0; i< nxmat; i++) {
        ...
        templist = cpeftnS(time, status,xvecCOL, currentlevels,  nused,  nvar,offset, weights,  strata,  eps, toler);
    ...
    SEXP outdel2;
    PROTECT(outdel2 = allocVector(REALSXP, 2));
    protected++;
    double *outdel;
    outdel =REAL(outdel2);
    outdel[0]=maxCPE;
    ...
    SEXP finallist, finalnames;
    PROTECT(finallist= allocVector(VECSXP, 1));
    protected++;
    SET_VECTOR_ELT(finallist, 0,outdel2);

    PROTECT(finalnames = allocVector(STRSXP, 1));
    protected++;
    SET_STRING_ELT(finalnames, 0, mkChar("max.cpe"));
    setAttrib(finallist, R_NamesSymbol, finalnames);
    unprotect(protected);
    return(finallist);
}
